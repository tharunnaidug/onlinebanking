...
export const checkTransactionLimit = async (req, res, next) => {
  const{id}=req.user;
  const { amount } = req.body; // Amount is the transaction amount
  ...
  try {
    ...
    if (parseFloat(transaction_count) >= account.max_daily_transactions) {
      return res.status(400).json({ message: 'Transaction limit exceeded for the day.' });
    }

    if (Number(total_amount)+parseFloat(amount) > account.transaction_limit_per_day) {
